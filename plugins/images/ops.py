...
def _prepare(image, s):
    """轉正、統一色彩模式、縮小;回傳 (圖片, 要保留的 EXIF 或 None)。"""
    ...
    image = _fit(_normalize(image), s.side)
...
def encode(image, target, s, exif=None):
    """把處理好的單張圖存成指定格式,回傳檔案內容。"""
    if target == "svg":
    ...
    if exif and target in ("jpg", "png", "webp", "avif", "heic"):
        extra["exif"] = exif
    if target == "jpg":
        _flatten(image).save(buf, "JPEG", quality=s.quality, optimize=True, progressive=True, **extra)
    elif target == "png":
        (_reduce_colors(image) if s.reduce_colors else image).save(buf, "PNG", optimize=True, **extra)
    elif target == "webp":
        image.save(buf, "WEBP", quality=s.quality, method=5, **extra)
    elif target == "heic":
        if pillow_heif is None:
            raise RuntimeError("缺少輸出 HEIC 的元件")
        image.save(buf, "HEIF", quality=s.quality, **extra)
    elif target == "avif":
        image.save(buf, "AVIF", quality=s.quality, **extra)
    elif target == "gif":
...
def _encode_animation(image, target, s):
    """動畫不做有損壓縮:每一格原樣保留(需要時縮小),WebP 用無損模式。"""
    frames, durations = [], []
    for frame in ImageSequence.Iterator(image):
        durations.append(frame.info.get("duration", image.info.get("duration", 100)))
        frames.append(_fit(frame.convert("RGBA"), s.side))
...
def convert(path, folder: Path, s: Settings):
    """轉換一張圖;回傳 (輸出檔, 附註)。"""
    path = Path(path)
    ...
    if target == "pdf":
        images_to_pdf([path], out, s)
        return out, ""

    if target == "svg" and source_format(path) == "svg":
    ...
    if frames > 1 and target in ANIMATED_FORMATS:
        untouched = (target == source_format(path) and max(image.size) <= (s.side or max(image.size))
                     and not (s.strip_meta and ("exif" in image.info or "xmp" in image.info)))
        if untouched:
            data = path.read_bytes()
            note = "動畫保持原樣"
        else:
            data = _encode_animation(image, target, s)
    else:
        if frames > 1:
            note = "動畫只保留第一格"
        image, exif = _prepare(image, s)
        data = encode(image, target, s, exif)
    out.write_bytes(data)
    return out, note


def _smallest_side(paths, s):
    """所有圖片(限制尺寸後)長邊最短的那一張的長邊;讀不了的圖不算。"""
    sides = []
    for path in paths:
        try:
            if source_format(path) == "svg":
                sides.append(max(open_image(path, s.side).size))
            else:
                size = max(Image.open(path).size)
                sides.append(min(size, s.side) if s.side else size)
        except Exception:
            continue
    return min(sides) if sides else None


def images_to_pdf(paths, out: Path, s: Settings, on_image=None, cancel=None):
    """依順序每張圖一頁合成 PDF;讀不了的圖會跳過。回傳 {第幾張: 錯誤訊息}。"""
    import pymupdf

    doc = pymupdf.open()
    failed = {}
    smallest = _smallest_side(paths, s) if s.pdf_page == "min" else None
    try:
        for index, path in enumerate(paths):
            ...
            if on_image is not None:
                on_image(index)
            try:
                image, _ = _prepare(open_image(path, s.side), s)
                if smallest:
