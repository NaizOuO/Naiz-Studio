...
- **標記與註解**：螢光筆、底線、刪除線（在文字上拖曳，自動對齊文字行）、文字框（直接在頁面上打字，支援注音，可以加背景色）、便利貼、直線、箭頭、方框、圓形（線條與填滿分開設定）、手繪；顏色選單分成主題色、標準色與最近使用，另可調整粗細、透明度、字型與字級
- **改字**：點一下文字修改整段，打字時自動重新換行，保留原本的對齊方式、縮排與行距；拖曳選取則只改其中幾個字。清空就是刪除
- **沿用原字型**：盡量使用 PDF 裡內嵌的原字型，新字和原字長得一樣；原字型裡沒有的字，改用相近的字型補上。儲存時原字會真正刪掉（不只是蓋住），別人選取、搜尋也找不到
...
- **PDF 轉 Word**：程式自己重建成真正的段落與表格，文字、字型、大小、顏色、圖片、分欄與縮排都會保留，可以直接接著編輯；有框線的表格照框線重建（含合併儲存格），用線條畫出來的圖表會整塊變成圖片
